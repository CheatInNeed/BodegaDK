...
export function applyCasinoMove(
    ...
    if (captureStackIds.length > 0) {
        ...
        for (const stackId of captureStackIds) {
            ...
            if (!stack) return `Capture stack ${stackId} is not on table`;
...
export function applyCasinoBuildMove(
    ...
    const legalCandidates = candidateTotals.filter((candidate) => playerHasTotalCardInHand(state.rules.valueMap, hand, move.handCard, candidate.total));
    if (legalCandidates.length === 0) return 'Build requires holding the resulting total card in hand';
...
export function applyCasinoMergeMove(
    ...
    if (!hasTotalInHand) return `Merge requires holding a ${newTotal}-value card in hand`;
    const baseStack = selectedStacks[0];
    baseStack.cards = selectedStacks.flatMap((stack) => stack.cards);
...
function playerHasTotalCardInHand(valueMap: CasinoValueMap, hand: string[], playedCard: string, total: number): boolean {
