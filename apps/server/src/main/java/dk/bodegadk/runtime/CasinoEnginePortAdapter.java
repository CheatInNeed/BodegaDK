    ...
    private CasinoAction parseAction(GameLoopService.ActionCommand command) {
        ...
        return switch (command.type()) {
            case "CASINO_PLAY_MOVE" -> {
                ...
                yield new CasinoAction.PlayMove(command.playerId(), handCard, readStringList(payload.path("captureStackIds")), readInt(payload.get("playedValue")));
            }
            case "CASINO_BUILD_STACK" -> {
                ...
                yield new CasinoAction.BuildStack(command.playerId(), handCard, targetStackId, readInt(payload.get("playedValue")));
            }
